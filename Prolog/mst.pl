...
% new_vertex/2 Adds a vertex V to a graph in the knowledge base. false
% if the graph is not in the knowledge base
...
% verteces/2 True when Vs is a list of every vertex in G

verteces(G, Vs) :- findall(vertex(G, V), vertex(G, V), Vs).
...
% new_arc/4 adds a weighted arc (an edge according to graph theory) to
% a graph G in the knowledge base

new_arc(G, U, V, Weight) :- graph(G),
			    vertex(G, U),
			    vertex(G, V),
			    arc(G, U, V, Weight), !.
...
% new_arc/3  adds a weighted arc with weight 1 (an edge according to
% graph theory) to a graph G in the knowledge base
...
% neighbors/3 true when V is a vertex in G and Ns is a list of all the
% arcs from V

neighbors(G, V, Ns) :- vertex(G, V),
