...
(defun heap-insert (heap-id k v)
  (let ((heap-rep (gethash heap-id *heaps*)))
    (cond ((eql nil (aref (heap-actual-heap heap-rep)
                          (heap-size heap-rep)))
           (setf (heap-size heap-rep)
                 (+ 1 (heap-size heap-rep)))
           (setf (aref (heap-actual-heap heap-rep) (heap-size heap-rep))
                 (list 'inf v))
           (heap-decrease-key (heap-actual-heap heap-rep)
                              (- 1 (heap-size heap-rep)) k))
    (T (error "heap is full")))))

(defun heap-decrease-key (heap i k)
  (cond ((or (equals (first (aref heap i)) 'inf)
             (equals (> (aref heap i)) k))
